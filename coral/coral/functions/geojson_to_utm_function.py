...
class GeoJSONToUTMPoint(BaseFunction):
    ...
    def save_utmpoint(self, tile, request, is_function_save_method=True):
        ...
        if geojsonValue != None:
            ...
            geometries = map(lambda x: x.geometry, geoJsFeatures)
...
"""
    ...
    def from_latlon(latitude, longitude):
        ...
        zone_number = GeoJSONToUTMPoint.latlon_to_zone_number(latitude, longitude)
        zone_letter = GeoJSONToUTMPoint.latitude_to_zone_letter(latitude)
    
        lon_rad = math.radians(longitude)
        central_lon = GeoJSONToUTMPoint.zone_number_to_central_longitude(zone_number)
        central_lon_rad = math.radians(central_lon)
    
        n = R / math.sqrt(1 - E * lat_sin**2)
        c = E_P2 * lat_cos**2
    
        a = lat_cos * GeoJSONToUTMPoint.mod_angle(lon_rad - central_lon_rad)
        a2 = a * a
        a3 = a2 * a
        a4 = a3 * a
        a5 = a4 * a
