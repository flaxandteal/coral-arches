  ...
  var viewModel = function (params) {
    ...
    if (drawFeatures.length > 0) {
      ...
      params.fitBoundsOptions = {
        padding: { top: padding, left: padding + 200, bottom: padding, right: padding + 200 }
      };
    }
    ...
    params.layers = ko.observable(extendedLayers.concat(selectFeatureLayers, geojsonLayers));
          ...
          promises.push(
            new Promise(function (resolve) {
              ...
              reader.onload = function (e) {
                var geoJSON;
                if (['json', 'geojson'].includes(extension)) geoJSON = JSON.parse(e.target.result);
                else
                  geoJSON = toGeoJSON.kml(
                    new window.DOMParser().parseFromString(e.target.result, 'text/xml')
                  );
                resolve(geoJSON);
              };
    ...
    self.updateBufferFeature = function () {
      ...
      if (bufferParams && bufferFeature) {
        ...
        window
          .fetch(arches.urls.buffer + '?filter=' + JSON.stringify(bufferParams))
