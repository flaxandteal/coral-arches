  ...
  return ko.components.register('views/components/reports/scenes/location', {
    viewModel: function (params) {
      ...
      self.adminAreasTableConfig = {
        ...self.defaultTableConfig,
        columns: [{ width: '50%' }, { width: '20%' }, { width: '20%' }, null]
      };
          ...
          const tileCards =
            rootCard === null
              ...
              : self.createCardDictionary(
                  rootCard.tiles().find((rootTile) => rootTile.tileid == tileid)?.cards
                );
      ...
      if (params?.compiled) {
        ...
        if (geometryNode) {
          ...
          self.geometryMetadata.compilerName(
            self.getNodeValue(
              geometryNode,
          ...
          self.geometryMetadata.compileDate(
            self.getNodeValue(
              geometryNode,
              'spatial record compilation',
          ...
          self.geometryMetadata.updateDate(
            self.getNodeValue(
              geometryNode,
          ...
          self.geometryMetadata.updaterName(
            self.getNodeValue(
              geometryNode,
              'spatial record update',
          ...
          self.geometryMetadata.authorizationDate(
            self.getNodeValue(
              geometryNode,
          ...
          self.geometryMetadata.authorizerName(
            self.getNodeValue(
              geometryNode,
              'spatial record authorization',
              'authorizer',
          ...
          self.geometryMetadata.typeOfAuthorization(
            self.getNodeValue(geometryNode, 'spatial record authorization', 'authorization type')
          );
          self.recordEditExists(self.observableValueSet(self.geometryMetadata));

          self.geometryScale.accuracy(
            self.getNodeValue(geometryNode, 'spatial accuracy qualifier')
          );
          self.geometryScale.basemap(
            self.getNodeValue(
              geometryNode,
              'current base map',
          ...
          self.geometryScale.coordinateSystem(
            self.getNodeValue(geometryNode, 'coordinate system', 'coordinate system value')
          );
        ...
        if (rawAddressesNode || Array.isArray(locationNode)) {
          const addressesNode = rawAddressesNode
            ...
            : locationNode.map((locationNode) =>
                self.getRawNodeValue(locationNode, self.dataConfig.addresses)
              );
          if (addressesNode?.length) {
            self.addresses(
              addressesNode.map((x) => {
                ...
                const buildingNumberSubStreet = self.getNodeValue(
                  x,
        ...
        if (
          (Array.isArray(nationalGridReferencesNode) && nationalGridReferencesNode?.length) ||
