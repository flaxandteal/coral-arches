define([
  ...
  'knockout',
  'uuid',
  'templates/views/components/workflows/application-area-workflow/app-area-address-step.htm'
  ...
  function viewModel(params) {
    ...
    this.fullAddress = ko.pureComputed(function () {
      var building = [self.buildingName(), self.buildingNumber(), self.buildingNumberSub()]
        .filter(Boolean)
        .join(' ');
    ...
    this.saveTile = function (tileDataObj, nodeGroupId, resourceid, tileid) {
      ...
      return window
        .fetch(arches.urls.api_tiles(tileid), {
          method: 'POST',
          credentials: 'include',
    ...
    params.form.save = function () {
      self
        .saveTile(nameTileDataObj(), nameNodegroupId, self.resourceid(), self.nameTileid())
        .then(function (data) {
          self.resourceid(data.resourceinstance_id);
          self.nameTileid(data.tileid);
          return self.saveTile(
            addressTileDataObj(),
            addressNodegroupId,
