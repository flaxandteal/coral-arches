  ...
  function viewModel(params) {
    ...
    this.resourceData.subscribe(function (val) {
      ...
      this.reportVals = {
        ...
        subject: {
          name: 'Subject',
          value: this.getResourceValue(currentCommunication, ['Subjects', 'Subject', '@value'])
        },
        type: {
          name: 'Type',
          value: this.getResourceValue(currentCommunication, ['Communication Type', '@value'])
        },
        date: {
          name: 'Date',
          value: this.getResourceValue(currentCommunication, ['Dates', 'Date', '@value'])
        },
        participants: {
          name: 'Participants',
          value: this.getResourceValue(currentCommunication, ['Attendees', '@value'])
        },
        relatedCondition: {
          name: 'Related Condition',
          value: this.getResourceValue(currentCommunication, ['Related Condition', '@value'])
        },
        notes: {
          name: 'Notes',
          value: this.getResourceValue(currentCommunication, [
            'Communication Notes',
        ...
        uploadedFiles: {
          name: 'Uploaded Files',
          value: this.getResourceValue(currentCommunication, ['Digital File(s)', '@value'])
        }
      };
    ...
    this.relatedResources.subscribe(function (val) {
      ...
      val['related_resources'].forEach(function (rr) {
        ...
        if (
          rr.graph_id == digitalObjectGraphId &&
          currentFileList.indexOf(rr.resourceinstanceid) > -1
        ) {
