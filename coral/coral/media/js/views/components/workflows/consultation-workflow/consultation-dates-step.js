define([
  ...
  'knockout',
  'knockout-mapping',
  'templates/views/components/workflows/consultation-workflow/consultation-dates-step.htm'
  ...
  function viewModel(params) {
    ...
    this.getResourceDisplayName = function (resourceids) {
      ...
      resourceids.forEach(function (id) {
        $.get(arches.urls.resource_descriptors + id, function (descriptors) {
          retStr == ''
            ? (retStr = descriptors.displayname)
            : (retStr += ', ' + descriptors.displayname);
    ...
    this.saveConsultationNameTile = function () {
      ...
      nameCardTile.data[self.consultationNameNodeId]({
        en: { direction: 'ltr', value: self.concatName() }
      });
    ...
    self.init = function () {
      ...
      if (!ko.unwrap(self.displayName) && !ko.unwrap(self.tile().data[self.targetDateNodeId])) {
        ...
        if (appAreas) {
          resourceids = appAreas.map(function (obj) {
    ...
    self.tile().data[self.logDateNodeId].subscribe(function (val) {
      ...
      if (val) {
        ...
        if (logDateVal != 'Invalid Date') {
          self.concatName(
            `Consultation for ${self.displayName()} on ${self.formatDate(logDateVal)}`
          );
