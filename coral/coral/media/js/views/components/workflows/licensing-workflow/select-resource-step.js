  ...
  function viewModel(params) {
      ...
      if (dat) {
        if (dat[0]) {
          ...
          self.fetchTileData(siteResourceId).then((data) => {
            archiveTiles = data.filter(
              (x) => x.nodegroup === '5f00ef7e-9f63-11ea-9db8-f875a44e0e11'
            );
    ...
    params.form.save = function () {
      self
        ...
        .save()
        .then(function () {
          activityResourceId = self
            .tile()
            .data['fbaebc8e-61ef-11ee-baf1-0242ac120004']()[0]
            .resourceId();
          activityTiles = self
            .fetchTileData(activityResourceId)
            .then((val) => {
              suitableTiles = val.filter(
                (tile) => tile.nodegroup === '5f00ef7e-9f63-11ea-9db8-f875a44e0e11'
              );
            })
            .then(function () {
