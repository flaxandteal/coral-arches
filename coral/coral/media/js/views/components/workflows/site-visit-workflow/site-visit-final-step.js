define([
  ...
  'viewmodels/alert',
  'templates/views/components/workflows/site-visit-workflow/site-visit-final-step.htm'
  ...
  function viewModel(params) {
    ...
    this.resourceData.subscribe(function (val) {
      ...
      this.reportVals = {
        ...
        date: {
          name: 'Date',
          value: this.getResourceValue(currentSiteVisit, [
            'Timespan of Visit',
            'Date of Visit',
            '@value'
          ])
        ...
        locatinDescription: {
          name: 'Visit Location Description',
          value: this.getResourceValue(currentSiteVisit, [
            'Location',
      ...
      try {
        this.reportVals.attendees = currentSiteVisit['Attendees'].map(function (attendee) {
          return {
            attendee: {
              name: 'Attendee',
              value: self.getResourceValue(attendee, ['Attendee', '@value'])
            },
            attendeeType: {
              name: 'Type',
              value: self.getResourceValue(attendee, ['Attendee Type', '@value'])
            }
          };
        });
      } catch (e) {
      ...
      try {
        this.reportVals.observations = currentSiteVisit['Observations'].map(function (obs) {
          return {
            observation: {
              name: 'observation',
              value: self.getResourceValue(obs, ['Observation', 'Observation Notes', '@value'])
            },
            observedBy: {
              name: 'observedBy',
              value: self.getResourceValue(obs, ['Observed by', '@value'])
            }
          };
        });
      } catch (e) {
      ...
      try {
        this.reportVals.recommendations = currentSiteVisit['Recommendations'].map(function (rec) {
          return {
            recommendation: {
              name: 'recommendation',
              value: self.getResourceValue(rec, [
                'Recommendation',
            ...
            recommendedBy: {
              name: 'recommendedBy',
              value: self.getResourceValue(rec, ['Recommended by', '@value'])
            }
          };
        });
      } catch (e) {
      ...
      try {
        this.reportVals.photographs = currentSiteVisit['Photographs'].map(function (photograph) {
          return {
            ...
            caption: {
              name: 'Caption',
              value: self.getResourceValue(photograph, ['Caption Notes', 'Caption Note', '@value'])
            },
            copyrightType: {
              name: 'Copyright Type',
              value: self.getResourceValue(photograph, ['Copyright', 'Copyright Type', '@value'])
            },
            copyrightHolder: {
              name: 'Copyright Holder',
              value: self.getResourceValue(photograph, ['Copyright', 'Copyright Holder', '@value'])
            },
            copyrightNotes: {
              name: 'Copyright Notes',
              value: self.getResourceValue(photograph, [
                'Copyright',
                'Copyright Note',
                'Copyright Note Text',
                '@value'
