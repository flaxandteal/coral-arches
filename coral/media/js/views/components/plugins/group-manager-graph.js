...
import cytoscapeElk from 'bindings/cytoscape-elk';
...
const viewModel = function(params) {
    ...
    this.activeGrouping = ko.isObservable(params.activeGrouping) ?
        params.activeGrouping :
        ko.observable(params.activeGrouping);
    this.activeGrouping.subscribe(console.error);
    ...
    var getResourceRelations = function(resourceId) {
        var url = `${arches.urls.root}groupmanager/${self.activeGrouping()}/${resourceId}?paginate=false&lang=${arches.activeLanguage}`;
    ...
    this.expandNode = function(node) {
        ...
        if (node.id) getResourceRelations(node.id)
            ...
            .then(function(result) {
                result = result.relatedResources;
                console.error(result.resource_relationships, node.id);
                const resource_relationships = result.resource_relationships;
                var elements = result.related_resources.concat(resource_relationships)
                    .map(function(data) {
    ...
    var updateFocusResource = function() {
        ...
        console.log("Focus", self.focusResourceId());
        if (resourceId) {
            var viz = self.viz();
            if (viz) {
            ...
            getResourceRelations(resourceId)
                ...
                .then(function(result) {
                    ...
                    result.resource_instance['total_relations'] = {
                        value: resource_relationships.length
                    };
                    var elements = [dataToElement(result.resource_instance)]
                        .concat(
                            result.related_resources.concat(resource_relationships)
                                .map(dataToElement)
                        );
                    elements = self.addMissingNodes(elements);
...
export default ko.components.register('group-manager-graph', {
        viewModel: viewModel,
