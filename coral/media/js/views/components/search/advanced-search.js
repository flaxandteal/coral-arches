...
import advancedSearchTemplate from 'templates/views/components/search/advanced-search.htm';
import 'bindings/let';
import 'bindings/key-events-click';


var componentName = 'advanced-search';
// must match `type` in arches/app/search/components/advanced_search.py details
var filterType = 'advanced-search-type';
const viewModel = BaseFilter.extend({
    initialize: function(options) {
        ...
        // ponytail: project override of the core component. Upstream fires this
        // 2.4MB fetch from initialize(), but every tabbed filter is instantiated
        // at page load (standard-search-view.htm binds `component:` inside a
        // `ko if: layoutType === 'tabbed'` — active tab is only a css class),
        // so /search blocks on it even when nobody opens Advanced. Deferred to
        // first tab activation. Still fetched eagerly when the URL already
        // carries facets, otherwise restoreState() would leave the UI empty on a
        // bookmarked/refreshed advanced search.
        const loadData = function() {
            if (self.dataRequested) { return; }
        ...
        // Body above is left at upstream's indentation on purpose: it keeps the
        // diff against arches core to just this wrapper, for the next re-sync.
        if (componentName in this.query()) {
            loadData();
        } else if (this.selectedTab && ko.isObservable(this.selectedTab)) {
            options.loading(false);
            if (this.selectedTab() === filterType) {
                loadData();
            } else {
                const tabSubscription = this.selectedTab.subscribe(function(tab) {
                    if (tab === filterType) {
            ...
            loadData();  // no tab observable to hang off — behave as upstream
        }
