...
import textWidgetTemplate from 'templates/views/components/widgets/regex-widget.htm';
...
const viewModel = function (params) {
  ...
  self.passedRegex = ko.computed(() => {
    const regex = new RegExp(self.regex());
    const currentText = self.currentText();
  ...
  self.currentText.subscribe((newValue) => {
    ...
    currentValue[currentLanguage.code].value = newValue?.[currentLanguage.code]
      ? newValue[currentLanguage.code]?.value
      : newValue;
...
export default ko.components.register('regex-widget', {
    viewModel: viewModel,
